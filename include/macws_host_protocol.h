...
// Versioned wire record for the future iOS-host -> macOS event bridge.
...
static_assert(sizeof(MacWSInputRecord) == 40, "MacWS input record ABI");
#else
_Static_assert(sizeof(MacWSFrameHeader) == 16, "MacWS frame header ABI");
_Static_assert(sizeof(MacWSInputRecord) == 40, "MacWS input record ABI");
#endif
