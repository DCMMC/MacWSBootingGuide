...
# The native Valve launcher uses fork after Network/XPC have initialized.
# Runtime crash steam_osx-2026-08-14-015741.ips proves its atfork child jumps
# into the non-executable xpc_dictionary_apply data page. Production therefore
# uses libmachook's atomic posix_spawn adapter for the top Web Helper and all
# Chromium descendants. The adapter preserves Valve's inherited descriptors,
# effective process group and explicit SIGCHLD-unblock contract. Keep the old
# fork path available only for an explicitly requested diagnostic comparison.
if [ "${MACWS_STEAM_NATIVE_BROWSER_LAUNCH:-0}" != "1" ]; then
    unset MACWS_STEAM_NATIVE_BROWSER_LAUNCH
fi
...
# Preserve Valve's top-level browser transport launch, then atomically spawn
# every renderer/network/GPU descendant. Steam's supported CEF switches keep
# only the client shell on software compositing; games retain Metal/AGX.
