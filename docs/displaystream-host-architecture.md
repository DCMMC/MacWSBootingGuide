...
| 缩放与精确操控 | 双指以手势中心放大，双指移动视口，双指双击或按钮恢复；输入坐标同步映射到裁剪后的源纹理 | 已实现并有数学单测 |
...
- 双指捏合：以手势中心为锚点，在 1×–4×之间连续放大。1×代表“铺满 Scene 的完整可见基准”，不会缩到产生黑边。
- 直接触控模式下双指拖动：移动被放大的视口；即使 1×时因宽高比产生基础裁剪，也可移动查看两侧内容。
- 触控板模式下双指拖动：优先作为 macOS 精确滚动；缩放后可用捏合改变锚点，妙控键盘指针移动到视口边缘时自动跟随。
- 恢复方式：双指双击，或者控制面板“恢复视图缩放”。恢复为 1×且中心点回到 `(0.5, 0.5)`。
- 当第二根手指加入时，Host 先取消正在进行的单指直接触控，再让 pinch/pan 识别，避免一次缩放误触 macOS 控件。
...
- 单指位置直接对应当前可见纹理中的 macOS 像素。
- down/move/up/cancel 保持同一 contact ID；发生 pinch/pan 竞争时必须发送 cancel。
- 适合经过触屏密度放大的按钮、列表、标签页和拖动目标。
...
| 模块 owner | 文件 | 负责的不变量 |
|---|---|---|
| 协议 owner | `include/macws_*_protocol.h`、`include/macws_viewport_math.h` | ABI、版本、上限、纯函数 |
...
- 无黑边 aspect-fill、1×–4×缩放、中心钳制、触摸映射纯 C 测试通过。
...
- 在 1×、2×、4×分别点击可见区域四角、中心和窗口标题栏控件；目标偏差不得随 zoom 增大。
- 捏合锚点下的控件在手势过程中保持在手指附近，不得跳到中心。
- 第二指加入时，macOS 必须收到第一指 cancel，不能留下卡住的 mouse-down。
- 双指移动到四个边界后，规范化 visible rect 始终在 `[0,1]` 内。
- 双指双击和控制面板按钮都恢复 1×/居中。
- 触控板模式的双指移动必须滚动内容，不移动视口；直接模式恰好相反。
...
- 最小尺寸遮罩、触屏密度、pinch/pan/reset、直接触控在三类应用通过。
