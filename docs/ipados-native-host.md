...
The CLI production profile now enables the currently required native-AGX
command/completion and capture compatibility by default. `--experimental`
remains an accepted compatibility alias; `--no-experimental` is reserved for
an explicit control experiment. High-overhead submit, resource and readback
recorders still require `--diagnostics` and are forbidden by the one-click
production preflight. The App transport remains a separate product milestone
and must not be used to infer the VNC production profile's state.
...
To exercise the native-AGX path without VNC, use the normal production
defaults and only remove the two clients. The script owns creation and cleanup
of its functional sentinels, including cleanup after a watchdog stop:
